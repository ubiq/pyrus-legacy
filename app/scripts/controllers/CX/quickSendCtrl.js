			...
			uiFuncs.generateTx(txData, function(rawTx) {
				if (!rawTx.isError) {
					uiFuncs.sendTx(rawTx.signedTx, function(resp) {
						if (!resp.isError) {
							$scope.sendTxStatus = $sce.trustAsHtml(globalFuncs.getSuccessText(globalFuncs.successMsgs[2] + "<br />" + resp.data + "<br /><a href='http://ubiqscan.io/tx/" + resp.data + "' target='_blank'> UBQ TX via UbiqScan.io </a>"));
							$scope.setBalance();
						} else {
